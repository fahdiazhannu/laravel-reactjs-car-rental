...
function ViewMobil(props)
{
    ...
    useEffect(() => {
        ...
        const mobil_slug = props.match.params.slug;
        axios.get( `/api/fetchmobil/${mobil_slug}`).then(res => {
            if(isMounted )
            {
                if(res.data.status === 200)
                {
                    setMobil(res.data.mobil_data.mobil);
                    setCategory(res.data.mobil_data.category);
                    setLoading(false);
                }
            ...
            return (
                ...
                <Card style={{ width: '16rem'}}>
                  ...
                  <Card.Body>
                    ...
                    <Card.Text>
                    <FontAwesomeIcon icon={faUsers} /> {item.kapasitas} Disukai
                    </Card.Text>
                  <Card.Footer>
